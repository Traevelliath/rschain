...
use regex::Regex;
use rschain_core::hex;
use rschain_core::utilities::crypto::keccak256;
use rschain_core::{to_hex_string, Address, AddressError, Error, PublicKey};
...
impl EthereumAddress {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>, Error> {
        ...
        let address = self.0.to_lowercase();
        let address = regex.replace_all(&address, "");
        Ok(hex::decode(address.as_bytes())?)
    }
...
impl FromStr for EthereumAddress {
    ...
    fn from_str(address: &str) -> Result<Self, Self::Err> {
        ...
        let address = regex.replace_all(&address_lowercase, "");

        if address.len() != 40 {
            return Err(AddressError::InvalidByteLength(address.len()));
        }

        let hash = to_hex_string(&keccak256(address.as_bytes()));
        let mut checksum_address = String::with_capacity(64);
        ...
        let chars =
            address
                ...
                .zip(hash.chars())
                .take(40)
                .map(|(addr_c, hash_c)| match hash_c {
                    char if nums.contains(&char) => addr_c.to_ascii_lowercase(),
                    _ => addr_c.to_ascii_uppercase(),
                });
        checksum_address.extend(chars);
...
mod tests {
    use super::*;
    use rschain_core::public_key::PublicKey;
