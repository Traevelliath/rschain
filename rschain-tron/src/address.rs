...
use ethabi::Token;
use hex::FromHex;
use rschain_core::{libsecp256k1, Address, AddressError, PublicKey};
use serde::Serialize;
...
impl TronAddress {
    ...
    pub fn from_token(token: Token) -> Self {
        let addr_token = token.into_address().unwrap_or_default();
        let mut raw_addr = [0x41_u8; 21];
        raw_addr[1..21].copy_from_slice(addr_token.as_bytes());

        Self::from_bytes(&raw_addr).to_owned()
    }
}
