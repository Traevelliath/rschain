...
use rschain_core::ethereum_types::U256;
use rschain_core::libsecp256k1::SecretKey;
use rschain_core::utilities::crypto;
use rschain_core::TransactionError;
use rschain_core::TransactionId;
use rschain_core::{libsecp256k1, Transaction};
use std::fmt;
use std::str::FromStr;
...
impl TronTransaction {
    pub fn sign_transaction(mut self, secret_key: &SecretKey) -> Result<Vec<u8>, TransactionError> {
        let hash = self.to_transaction_id()?.txid;
...
mod tests {
    ...
    use rschain_core::{libsecp256k1, Address, PublicKey};
    ...
    fn test_broadcast_trc20_transfer() {
        ...
        let addr_from = TronAddress::from_secret_key(&secret_key, &TronFormat::Standard)
            .unwrap()
            .to_base58();
        let addr_to = "TTxnd7cR8GDyCV5W1nKUAZiXfKmrMoLoLo";
